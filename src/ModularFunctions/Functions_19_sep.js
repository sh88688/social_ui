...
export const SELECT_OPTION_SETTER = ( SELECT_FIELD, DISPLAY_FIELD, VALUE_FIELD, RESULT, COMPONENT) => {
  
  //SELECT_FIELD : select field's ID in dataform
  //DISPLAY_FIELD : API Result's display key
  //VALUE_FIELD : API Result's value key
  //RESULT : API Result
  //COMPONENT : this [refrence]
  let COPIED_FORM = null;

  if(SELECT_FIELD.MODE === 1)
  {
    COPIED_FORM = JSON.parse(COMPONENT.DEFAULT_JSON);
    let INDEX = COPIED_FORM.findIndex(e => e.id === SELECT_FIELD.MULTI_ID);
    console.log("==>multi  ",COPIED_FORM,INDEX);
    COPIED_FORM = COPIED_FORM[INDEX].config.fields[0];
    console.log("== multi == > ",COPIED_FORM, COPIED_FORM.findIndex(e => e.id === SELECT_FIELD.ID));
  }
  else{
     COPIED_FORM = JSON.parse(COMPONENT.DEFAULT_JSON);
  }
  //copying Default JSON
  // const COPIED_FORM = JSON.parse(COMPONENT.DEFAULT_JSON);
  const DATA_INDEX = COPIED_FORM.findIndex(e => e.id === SELECT_FIELD.ID);
  const DATA_FIELD = {...COPIED_FORM[DATA_INDEX]};
  // console.log('fun ==> ',DATA_FIELD);
  const OPTIONS = RESULT.dataInfo.map((e)=> { return { displayValue : e[DISPLAY_FIELD], value : e[VALUE_FIELD] }});
  DATA_FIELD.config.options = OPTIONS;
  COPIED_FORM[DATA_INDEX] = DATA_FIELD;
  //update Default JSON With Loaded Select Options
  COMPONENT.DEFAULT_JSON = JSON.stringify(COPIED_FORM);
  COMPONENT.setState({  dataForm : COPIED_FORM});
  }
